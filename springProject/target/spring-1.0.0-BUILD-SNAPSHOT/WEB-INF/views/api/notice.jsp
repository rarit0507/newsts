		  ...
		  <div class="modal-content">
			...
			<div class="card">
			  ...
			  <div class="card-body">       
					...
					<input type="hidden" value="" id="updateNo"/>	<!-- 공지번호 숨겨 같이 보냄 -->
				  <div class="form-group">
					<label>작성자</label>
	...
	*/
	...
	function update() {
		...
		$.ajax({
			url : 'notice',
			type : 'put',
			data : JSON.stringify(updateData),	//JSON으로 데이터 가공할거고
			contentType : 'application/json', 	//내가 이 타입으로 데이터를 보내겠다
			success : result => {
	...
	$('#content').on('click', '.noticeEl', e => {	//클릭하면, 부모요소를, 이벤트발생
		...
		$.ajax({
			...
			success : response => {
				...
				//258행에서는 조회된 Notice객체에 대한 데이터를 모두 사용할 수가 있음
